"""
워드 파일 임베딩 API 엔드포인트
RAG 워크플로우를 통한 워드 문서 처리 및 검색 기능
"""
...
# 라우터 생성
router = APIRouter(prefix="/api/word-embedding", tags=["Word Embedding"])

# 워드 임베딩 서비스 인스턴스 (전역)
word_embedding_service = None


def get_word_embedding_service() -> WordEmbeddingService:
    """워드 임베딩 서비스 인스턴스 반환"""
    global word_embedding_service
    if word_embedding_service is None:
        try:
            word_embedding_service = WordEmbeddingService()
            logger.info("워드 임베딩 서비스 초기화 완료")
        except Exception as e:
            logger.error(f"워드 임베딩 서비스 초기화 실패: {e}")
            raise HTTPException(status_code=500, detail=f"서비스 초기화 실패: {str(e)}")
    return word_embedding_service
...
async def upload_word_document(
    ...
    try:
        ...
        # 문서 처리
        result = service.process_word_document(temp_file_path, metadata)
        ...
        logger.error(f"워드 문서 업로드 처리 실패: {e}")
